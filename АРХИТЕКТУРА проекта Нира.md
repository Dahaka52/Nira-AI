...
- **`GigaAMSTT` (`gigaam.py`):**
  - Основной локальный STT на базе `sherpa-onnx` (модель `GigaAM-v3 NeMo CTC int8`).
  - Сверхбыстрый инференс на CPU (<50 мс, RTF ~0.02), оптимизирован для русского языка.
  - Встроенная капитализация, пунктуация, авто-нормализация громкости (AGC), поддержка Hotwords и словаря омофонов.
  - Не занимает VRAM видеокарты, оставляя всю память RTX 5070 для локальной LLM.
- **`OpenAISTT` (`openai_api.py`):**
  - Облачный резервный STT через Groq API (`whisper-large-v3-turbo`, задержка ~120-180 мс).
