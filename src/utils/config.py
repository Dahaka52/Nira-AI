...
class Config(metaclass=Singleton):
    ...
    t2t_active_id: str = None
    t2t_strict_active_id: bool = False
    t2t: dict = dict()
    ...
    llamacpp_cache_type_k: str = "f16" # f16, f32, q8_0, q4_0, etc
    ...
    def save(self, config_name: str):
        filename = config_name if config_name.endswith((".yaml", ".yml")) else f"{config_name}.yaml"
        filepath = portable_path(os.path.join(self.CONFIG_DIR, filename))
        with open(filepath, 'w', encoding="utf-8") as f:
            yaml.dump(self.get_config_dict(), f, allow_unicode=True)
