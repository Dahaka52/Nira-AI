...
class Prompter(metaclass=Singleton):
    def __init__(self):
        ...
        # [OPTIMIZE] Smart Cache for prompt files
        self._prompt_cache: Dict[str, Dict[str, Any]] = {
            "instruction": {"content": "", "sig": (0, 0)},
            "character": {"content": "", "sig": (0, 0)},
            "scene": {"content": "", "sig": (0, 0)}
        }
    ...
    def _get_cached_prompt(self, cache_key: str, subdir: str, filename: str) -> str:
        """Вспомогательная функция для умного кэширования файлов промптов"""
        file_path = portable_path(os.path.join(
            Config().PROMPT_DIR,
            subdir,
            filename
        ))
        
        cache = self._prompt_cache.get(cache_key)
        try:
            st = os.stat(file_path)
            sig = (st.st_mtime_ns, st.st_size)

            # Если файл не изменился и кэш не пуст — отдаем из памяти
            ...
            logging.error(f"Prompt cache read failed for {file_path}: {e}", exc_info=True)
            return cache["content"] if cache else ""
