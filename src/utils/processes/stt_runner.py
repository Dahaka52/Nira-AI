...
class STTProcessRunner:
    """
    Small helper around ProcessManager for STT sidecar lifecycle.
    Keeps operation code clean and gives a single place for health/restart logic.
    """
    ...
    async def ensure_healthy(self) -> None:
        ...
        for attempt in range(1, self.startup_retries + 1):
            ...
            try:
                ...
                last_exc = exc
                logging.warning(
                    "STTProcessRunner ensure_healthy attempt %s/%s failed: %s",
                    attempt,
        ...
        if last_exc is not None:
            raise last_exc
        raise RuntimeError(f"STT process {self.process_type.value} failed health checks after retries.")
