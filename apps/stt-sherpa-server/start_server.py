...
"""
    ...
    async def handle_connection_impl(
        ...
        while True:
            ...
            if samples is None:
                logging.info(f"Received Done from {socket.remote_address}")
                break

            # TODO(fangjun): At present, we assume the sampling rate
            # of the received audio samples equal to --sample-rate
            logging.info(f"Accepting {len(samples)} samples from {socket.remote_address}")
...
if __name__ == "__main__":
    log_filename = "logs/log-streaming-server"
