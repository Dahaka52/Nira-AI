...
8. Мульти-люди/стрим: поддержка разных источников вывода (UI, Discord, OBS, виртуальный микрофон).
...
5. UI пока не воспроизводит `audio_chunk`, только читает TTS метрики. - !Веб UI не должен воспроизводить. Вывод звука планируется в виртуальном кабеле/voicemeter и дальнейшее подключение к Discord/OBS.
...
### Шаг 7. UI аудио-воспроизведение

Файлы:

1. `apps/nira-web/src/App.tsx`.
2. `apps/nira-web/src/*` (аудио буфер/плеер).

Сделать:

1. Буферизация `audio_chunk`.
2. Воспроизведение PCM в WebAudio.
3. Обработка underrun/overrun.

Критерий: пользователь слышит поток в UI без щелчков и рассинхронизации.
